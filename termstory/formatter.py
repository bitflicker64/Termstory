...
def format_rage_quit_signatures(
    signatures: List[Dict[str, Any]], total_events: int
) -> str:
    """Format rage-quit signatures for display in the CLI.

    Args:
        signatures: List of rage-quit signature dictionaries
        total_events: Total number of rage-quit events analyzed
    ...
    if not signatures:
        return (
            "😤 No rage-quit signatures found! You're either very"
            " consistent or never close your terminal."
        )

    output_lines = []
    output_lines.append("🔥 Rage-Quit Signatures 🔥")
    output_lines.append("=" * 50)
    output_lines.append("")
    output_lines.append(f"📊 Analyzed {total_events} rage-quit events")
    output_lines.append("")

    for i, sig in enumerate(signatures[:10], 1):  # Show top 10
        command = sig["command"]
        count = sig["count"]

        # Determine emoji based on command type
        emoji = "🤔"
        lower_cmd = command.lower()
        if "git push" in lower_cmd:
            emoji = "🚀"
        elif "git" in lower_cmd:
            emoji = "📝"
        elif "kill" in lower_cmd or "pkill" in lower_cmd:
            emoji = "💀"
        elif "exit" in lower_cmd or "logout" in lower_cmd:
            emoji = "🚪"
        elif "npm" in lower_cmd or "yarn" in lower_cmd or "pnpm" in lower_cmd:
            emoji = "📦"
        elif "docker" in lower_cmd or "kubectl" in lower_cmd:
            emoji = "🐳"
        elif "vim" in lower_cmd or "nvim" in lower_cmd or "code" in lower_cmd:
            emoji = "✍️"
        elif "sudo" in lower_cmd:
            emoji = "👑"
        elif "python" in lower_cmd or "node" in lower_cmd or "java" in lower_cmd:
            emoji = "💻"

        percentage = round((count / total_events) * 100, 1) if total_events > 0 else 0

        output_lines.append(f"{emoji} #{i}: {command}")
        output_lines.append(
            f"   • Occurrences: {count} ({percentage}% of rage-quits)"
        )
        output_lines.append("")

    output_lines.append("=" * 50)
    output_lines.append(
        "💡 Your top rage-quit is what ends your workday most often!"
    )
    output_lines.append("   (Based on 12+ hour gaps between terminal commands)")
